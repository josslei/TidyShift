...
def make_random_object_loader(bc: BulletClient, obj_dir: pathlib.Path) -> Callable[[], int]:
...
def setup_simulation(bc: BulletClient) -> int:
    """Reset the simulation environment and load the table and plane.

    Returns:
        int: The ID of the table object
    """
    bc.resetSimulation()
    bc.setGravity(0, 0, -9.81)
    bc.setAdditionalSearchPath(pybullet_data.getDataPath())
    table_id: int = bc.loadURDF("table/table.urdf", [0, 0, 0.7])
    _ = bc.loadURDF("plane.urdf")
    return table_id
...
def render_pointcloud(
    ...
    img = bc.getCameraImage(
        ...
        view_matrix,
        projection_matrix,
        renderer=bc.ER_BULLET_HARDWARE_OPENGL,
        flags=bc.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX,
    )
    ...
    # Extract object IDs
    object_ids: np.ndarray = segmentation_mask.reshape(-1) >> 24
    return points, object_ids


def save_scene(
    out_dir: pathlib.Path, scene_idx: int, language: str, obj_pointclouds: Dict[int, np.ndarray]
) -> None:
...
class GenerateSceneWorker:
    ...
    def __call__(self, scene_idx: int) -> int:
        ...
        setup_simulation(bc)
        _load_random_object: Callable[[], int] = make_random_object_loader(bc, self.obj_dir)
        obj_ids: List[int] = [
        ...
        obj_clouds: Dict[int, np.ndarray] = {
            oid: points[obj_mask == oid] for oid in np.unique(obj_mask) if oid in obj_ids
        }
...
if __name__ == "__main__":
    main(pathlib.Path("GSO_extracted"), pathlib.Path("scenes_GSO"), 1)
